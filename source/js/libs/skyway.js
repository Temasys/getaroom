...
if (navigator.mozGetUserMedia) {
  ...
  setDataChannelEvents = function(dc,self) {
    dc.onerror = function(error){ console.log("Failed retrieveing dataChannel"); };
    ...
    dc.onmessage = function() {
      ...
      self._readFile(JSON.parse(event.data));
    };
    return dc;
  };
  ...
  Skyway.prototype._processSingleMsg = function (msg) {
    ...
    case 'bye':
      ...
      break;
    case 'chat':
      if(msg.fid) this._trigger('sentFileStatus', msg);
  ...
  Skyway.prototype._offerHandler = function (msg) {
    ...
    pc.ondatachannel = function (evt) {
      var odc = evt.channel;
      window.dataChannels[targetMid+"_offer"] = window.setDataChannelEvents(odc,self);
      window.dataChannels[targetMid+"_offer"].ready = true;
      console.log("dataChannels['" + targetMid + "_offer'] communicating");
    };
  ...
  Skyway.prototype._doAnswer = function (targetMid) {
    ...
    var self = this;
    if (pc) {
      var adc = pc.createDataChannel(targetMid,{reliable:false});
      window.dataChannels[targetMid+"_answer"] = window.setDataChannelEvents(adc,self);
      console.log("API - dataChannels['" + targetMid + "_answer'] started");
  ...
  Skyway.prototype._doCall = function (targetMid) {
    ...
    var odc = pc.createDataChannel(targetMid, {reliable:false});
    window.dataChannels[targetMid+"_offer"] = window.setDataChannelEvents(odc,self);
    console.log("API - dataChannels['" + targetMid + "_offer'] started");
  ...
  Skyway.prototype._answerHandler = function (msg) {
    ...
    pc.ondatachannel = function(evt) {
      var adc = evt.channel;
      window.dataChannels[targetMid+"_answer"] = window.setDataChannelEvents(adc,self);
      window.dataChannels[targetMid+"_answer"].ready = true;
      console.log("API - dataChannels['" + targetMid + "_answer'] communicating");
    };
  };
  ...
  /**
    * Sending of Files as binaryData
    * @method sendFile
    * @protected
    * @param {File} fileInfo, {blob} binaryData
    */
  Skyway.prototype.sendFile = function(fileInfo,dataURL){
    var msg = {
      type: 'file',
      fid: this._user.id + 
        (((new Date()).toISOString().replace(/-/g,"_")).replace(/:/g,"_")).replace('.',"_"),
    ...
    for(var channel in window.dataChannels){
      if(window.dataChannels[channel].ready){
        window.dataChannels[channel].send(JSON.stringify(msg));
      }
    }
    this._trigger('receivedFile',msg);
  };
  ...
  /**
    ...
    * @param {JSON} data
    */
  Skyway.prototype._readFile = function(data) {
    if(data.type==='file') { 
      this._trigger('receivedFile',data);
      this._sendMessage({
        type: 'chat', // What a playcheat
        mid: this._user.id,
        rid: this._room.id,
        fid:data.fid,
        user:this._user.displayName
    ...
    else { console.log("Text: " + data.data); }
  };
