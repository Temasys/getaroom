/*! skylinkjs - v0.6.26 - Tue Oct 17 2017 15:28:31 GMT+0800 (+08) */
...
(function(globals) {
   ...
   * @param {Number} RETRY     <small>Value <code>1.5</code></small>
   *   The value of the state when <code>init()</code> is retrying authenticating of App Key provided after
   *   having no response for the amount of time configured as <code>options.apiTimeout</code>.
  ...
  Skylink.prototype._createSession = function (roomName) {
    var self = this;
    var session = {};
    ...
    session.authTokens = {};
    session.authPath =
      self._initOptions.roomServer + '/api/' + self._initOptions.appKey + '/' + roomName +
      (self._initOptions.credentials ? '/' + self._initOptions.credentials.startDateTime + '/' +
      self._initOptions.credentials.duration + '?cred=' + self._initOptions.credentials.credentials : '') +
      (self._initOptions.credentials ? '&' : '?') + 'rand=' + Date.now();
    session.authRetries = 0;
    session.priorityWeight = 0;
    session.iceServers = [];
  
    // Session socket connection to Signaling server.
    session.socket = null;
    session.socketOptions = {};
    session.socketServer = 'signaling.temasys.io';
    session.socketPorts = {
      'http:': [80, 3000],
      'https:': [443, 3443]
    };
    session.socketTransports = ['polling'];
    session.socketLatency = {
      timestamp: null,
      latency: 0
    };
  
    // Session socket message buffer.
    session.buffer = {
      target: [],
      broadcast: []
    };
    session.bufferTimestamp = null;
    session.bufferInterval = null;
    session.bufferStamps = {
      updateUserEvent: 0,
      muteAudioEvent: 0,
      muteVideoEvent: 0
    };
    ...
    if (window.WebSocket) {
      session.socketTransports.splice(0, 0, 'websocket');
    }
  
    if (typeof self._initOptions.socketServer === 'string')
  
  
    /**
     * Function that authenticates the current session App key.
     */
    session.authenticate = function () {
      ...
      xhr.onload = function () {
        ...
        if (response.success) {
          log.info([null, 'Session', roomName, 'Authentication: Success ->'], response);
  
          session.roomId = response.room_key;
          session.socketServer = response.ipSigserver || session.socketServer;
          session.socketPorts['http:'] = Array.isArray(response.httpPortList) && response.httpPortList.length > 0 ?
            response.httpPortList : session.socketPorts['http:'];
          session.socketPorts['https:'] = Array.isArray(response.httpsPortList) && response.httpsPortList.length > 0 ?
            response.httpsPortList : session.socketPorts['https:'];
          session.authTokens = response;
          session.states.readyState = self.READY_STATE_CHANGE.COMPLETED;
          self._trigger('readyStateChange', self.READY_STATE_CHANGE.COMPLETED, null, session.roomName);
  
        } else {
          log.error([null, 'Session', roomName, 'Authentication: Failed ->'], response);
  
          session.states.readyState = self.READY_STATE_CHANGE.ERROR;
          self._trigger('readyStateChange', self.READY_STATE_CHANGE.ERROR, {
  
            status: status || response.error,
            content: new Error(response.info || 'Failed authenticating with response (' + response.error + ') from Auth server'),
            errorCode: response.error || self.READY_STATE_CHANGE_ERROR.INVALID_XMLHTTPREQUEST_STATUS
      ...
      xhr.onerror = function () {
        ...
        hasCompleted = true;      
        log.error([null, 'Session', roomName, 'Authentication: Caught exception']);
  
        session.states.readyState = self.READY_STATE_CHANGE.ERROR;
        self._trigger('readyStateChange', self.READY_STATE_CHANGE.ERROR, {
  
          status: xhr.status || null,
          content: new Error('Failed authenticating due to Auth server connection errors'),
          errorCode: self.READY_STATE_CHANGE_ERROR.XML_HTTP_REQUEST_ERROR
      ...
      try {
        log.debug([null, 'Session', roomName, 'Authentication: Loading']);
  
        session.states.readyState = self.READY_STATE_CHANGE.LOADING;
        self._trigger('readyStateChange', self.READY_STATE_CHANGE.LOADING, null, session.roomName);
  
        xhr.open('GET', (self._initOptions.forceSSL ? 'https:' : window.location.protocol) + session.authPath, true);
        xhr.send();
      ...
      setTimeout(function () {
        ...
        if (session.authRetries <= 4) {
          session.authRetries++;
          log.debug([null, 'Session', roomName, 'Authentication: Retrying after ' + session.authRetries + ' times']);
  
          session.states.readyState = self.READY_STATE_CHANGE.RETRY;
          self._trigger('readyStateChange', self.READY_STATE_CHANGE.RETRY, null, session.roomName);
  
          session.authenticate();
          return;
        }
  
        log.error([null, 'Session', roomName, 'Authentication: Retries aborted after ' + session.authRetries + ' times']);      
  
        session.states.readyState = self.READY_STATE_CHANGE.ERROR;
        self._trigger('readyStateChange', self.READY_STATE_CHANGE.ERROR, {
  
          status: xhr.status || null,
          content: new Error('Failed authenticating as there is no response from the Auth server'),
          errorCode: self.READY_STATE_CHANGE_ERROR.XML_HTTP_NO_REPONSE_ERROR
    ...
    session.connect = function () {
      var useOptions = clone(session.socketOptions);
      var fallbackType = useOptions.port ? (useOptions.protocol === 'https:' ?
        (useOptions.transport === 'websocket' ? self.SOCKET_FALLBACK.FALLBACK_SSL_PORT : self.SOCKET_FALLBACK.LONG_POLLING_SSL) :
        (useOptions.transport === 'websocket' ? self.SOCKET_FALLBACK.FALLBACK_PORT : self.SOCKET_FALLBACK.LONG_POLLING)
      ) : self.SOCKET_FALLBACK.NON_FALLBACK;
  
      if (typeof self._initOptions.socketServer === 'string') {
        var parts = self._initOptions.socketServer.split(':');
        useOptions.serverUrl = self._initOptions.socketServer;
        useOptions.protocol = ['https', 'wss'].indexOf(parts[0]) > -1 ? 'https:' : 'http:';
        useOptions.port = parseInt((parts[2] || '').split('?')[0] || (useOptions.protocol === 'https:' ? '443' : '80'), 10);
  
      } else {
        var useProtocol = (self._initOptions.socketServer && self._initOptions.socketServer.protocol) ||
          (self._initOptions.forceSSL ? 'https:' : window.location.protocol);
        var useServer = (self._initOptions.socketServer && self._initOptions.socketServer.url) || session.socketServer;
        var usePorts = self._initOptions.socketServer && Array.isArray(self._initOptions.socketServer.ports) &&
          self._initOptions.socketServer.ports.length > 0 ? self._initOptions.socketServer.ports : session.socketPorts[useProtocol];
        ...
        useOptions.serverUrl = useProtocol + '//' + useServer + ':' + useOptions.port;
      }
      ...
      fallbackType = useProtocol === 'https:' ?
      (window.WebSocket ? self.SOCKET_FALLBACK.FALLBACK_SSL_PORT : self.SOCKET_FALLBACK.LONG_POLLING_SSL) :
      (window.WebSocket ? self.SOCKET_FALLBACK.FALLBACK_PORT : self.SOCKET_FALLBACK.LONG_POLLING);
    ...
    log.debug([null, 'Session', roomName, 'Initialised']);
    return session;
  };
